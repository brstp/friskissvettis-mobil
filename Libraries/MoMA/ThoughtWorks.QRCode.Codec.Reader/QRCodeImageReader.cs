...
namespace ThoughtWorks.QRCode.Codec.Reader
{
	...
	public class QRCodeImageReader
	{
		...
		public virtual QRCodeSymbol getQRCodeSymbol(int[][] image)
		{
			...
			catch (FinderPatternNotFoundException var_2_8E)
			{
			...
			catch (IndexOutOfRangeException var_18_309)
			{
		...
		public virtual QRCodeSymbol getQRCodeSymbolWithAdjustedGrid(Point adjust)
		{
			...
			catch (IndexOutOfRangeException var_3_A7)
			{
				throw new SymbolNotFoundException("Sampling grid exceeded image boundary");
			}
			return new QRCodeSymbol(moduleMatrix);
		...
		internal virtual SamplingGrid getSamplingGrid(FinderPattern finderPattern, AlignmentPattern alignmentPattern)
		{
			...
			for (int i = 0; i < num2; i++)
			{
				for (int j = 0; j < num2; j++)
				{
					...
					modulePitch.top = this.getAreaModulePitch(point, point2, num3 - 1);
					modulePitch.left = this.getAreaModulePitch(point, point3, num4 - 1);
					modulePitch.bottom = this.getAreaModulePitch(point3, point4, num3 - 1);
					modulePitch.right = this.getAreaModulePitch(point2, point4, num4 - 1);
					line.setP1(point);
					line2.setP1(point);
					line.setP2(point3);
					line2.setP2(point2);
