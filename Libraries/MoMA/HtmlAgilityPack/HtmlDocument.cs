...
namespace HtmlAgilityPack
{
	public class HtmlDocument : IXPathNavigable
	{
		...
		public Encoding DetectEncoding(string path)
		{
			...
			StreamReader streamReader = new StreamReader(path, this.OptionDefaultStreamEncoding);
			Encoding result = this.DetectEncoding(streamReader);
			streamReader.Close();
			return result;
		}
		public Encoding DetectEncoding(TextReader reader)
		{
			...
			{
				this._openednodes = new Hashtable();
			}
			else
			{
		...
		public Encoding DetectEncodingHtml(string html)
		{
			...
			Encoding result = this.DetectEncoding(stringReader);
			stringReader.Close();
			return result;
		}
		...
		public void Load(TextReader reader)
		{
			...
			if (this.OptionCheckSyntax)
			{
				foreach (HtmlNode htmlNode in this._openednodes.Values)
				{
		...
		{
			StreamWriter streamWriter = new StreamWriter(filename, false, this.GetOutEncoding());
		...
		internal Encoding GetOutEncoding()
		{
			...
			return this.OptionDefaultStreamEncoding;
		}
		...
		private HtmlParseError AddError(HtmlParseErrorCode code, int line, int linePosition, int streamPosition, string sourceText, string reason)
		{
			...
			this._parseerrors.Add(htmlParseError);
		...
		private void CloseCurrentNode()
		{
			...
			bool flag = false;
			HtmlNode htmlNode = (HtmlNode)this._lastnodes[this._currentnode.Name];
			if (htmlNode == null)
			{
				if (HtmlNode.IsClosedElement(this._currentnode.Name))
				{
					...
					if (this._lastparentnode != null)
					{
						...
						{
							if (htmlNode3.Name == this._currentnode.Name && !htmlNode3.HasChildNodes)
							{
								htmlNode2 = htmlNode3;
								break;
							}
						...
						if (htmlNode2 != null)
						{
							while (stack.Count != 0)
							{
								HtmlNode htmlNode4 = (HtmlNode)stack.Pop();
				...
				else
				{
					...
					{
						HtmlNode htmlNode5 = this.CreateNode(HtmlNodeType.Text, this._currentnode._outerstartindex);
						htmlNode5._outerlength = this._currentnode._outerlength;
						((HtmlTextNode)htmlNode5).Text = ((HtmlTextNode)htmlNode5).Text.ToLower();
						if (this._lastparentnode != null)
						{
							this._lastparentnode.AppendChild(htmlNode5);
						}
					}
					else
					{
			...
			{
				if (this.OptionFixNestedTags && this.FindResetterNodes(htmlNode, this.GetResetters(this._currentnode.Name)))
				{
				...
				if (!flag)
				{
					this._lastnodes[this._currentnode.Name] = htmlNode._prevwithsamename;
		...
		{
			HtmlNode htmlNode = (HtmlNode)this._lastnodes[name];
			if (htmlNode == null)
			{
				return null;
			}
		...
		private void Parse()
		{
			...
			while (this._index < this._text.Length)
			{
				...
				case HtmlDocument.ParseState.Comment:
					if (this._c == 62 && (!this._fullcomment || (this._text[this._index - 2] == '-' && this._text[this._index - 3] == '-')))
					{
						if (!this.PushNodeEnd(this._index, false))
						{
							this._index = this._text.Length;
				...
				case HtmlDocument.ParseState.ServerSideCode:
					if (this._c == 37 && this._index < this._text.Length && this._text[this._index] == '>')
					{
				...
				case HtmlDocument.ParseState.PcData:
					...
					{
						int num2 = (int)this._text[this._index - 1 + 2 + this._currentnode.Name.Length];
		...
		private bool PushNodeEnd(int index, bool close)
		{
			...
			else
			{
				if (this._currentnode._starttag && this._lastparentnode != this._currentnode)
				{
					...
					HtmlNode prevwithsamename = (HtmlNode)this._lastnodes[this._currentnode.Name];
			...
			{
				if (this.OptionStopperNodeName != null && this._remainder == null && string.Compare(this._currentnode.Name, this.OptionStopperNodeName, true) == 0)
				{
					this._remainderOffset = index;
					this._remainder = this._text.Substring(this._remainderOffset);
		...
		private void ReadDocumentEncoding(HtmlNode node)
		{
			...
			if (string.Compare(htmlAttribute.Value, "content-type", true) != 0)
			{
				return;
			}
			...
			if (htmlAttribute2 != null)
			{
				...
				if (!string.IsNullOrEmpty(text))
				{
					...
					if (this._streamencoding != null && this._declaredencoding.WindowsCodePage != this._streamencoding.WindowsCodePage)
					{
