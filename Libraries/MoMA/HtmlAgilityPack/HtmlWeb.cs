...
namespace HtmlAgilityPack
{
	public class HtmlWeb
	{
		...
		public static string GetContentTypeForExtension(string extension, string def)
		{
			...
			{
				if (HtmlWeb.MimeTypes.ContainsKey(extension))
				{
					result = HtmlWeb.MimeTypes[extension];
				}
				else
				{
					result = def;
			...
			if (!SecurityManager.IsGranted(new DnsPermission(PermissionState.Unrestricted)))
			{
				try
				{
					...
					registryKey = registryKey.OpenSubKey(extension, false);
					if (registryKey != null)
					{
						result = (string)registryKey.GetValue("", def);
					}
				}
				catch (Exception)
				{
		...
		public static string GetExtensionForContentType(string contentType, string def)
		{
			...
			if (!SecurityManager.IsGranted(new RegistryPermission(PermissionState.Unrestricted)))
			{
				...
				{
					foreach (KeyValuePair<string, string> current in HtmlWeb.MimeTypes)
					{
						if (current.Value == contentType)
						{
			...
			if (SecurityManager.IsGranted(new RegistryPermission(PermissionState.Unrestricted)))
			{
				...
				{
					RegistryKey registryKey = Registry.ClassesRoot;
					registryKey = registryKey.OpenSubKey("MIME\\Database\\Content Type\\" + contentType, false);
					if (registryKey != null)
					{
						result = (string)registryKey.GetValue("Extension", def);
					}
				}
				catch (Exception)
				{
		...
		public string GetCachePath(Uri uri)
		{
			...
			{
				result = Path.Combine(this._cachePath, (uri.Host + uri.AbsolutePath).Replace('/', '\\'));
			}
			return result;
		}
		...
		public HtmlDocument Load(string url, string method)
		{
			...
			else
			{
				...
				htmlDocument.DetectEncodingAndLoad(url, this._autoDetectEncoding);
			}
		...
		private void SaveCacheHeaders(Uri requestUri, HttpWebResponse resp)
		{
			...
			foreach (string text in resp.Headers)
			{
