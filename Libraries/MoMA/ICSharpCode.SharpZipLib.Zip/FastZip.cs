	...
	public class FastZip
	{
		...
		public void CreateZip(Stream outputStream, string sourceDirectory, bool recurse, string fileFilter, string directoryFilter)
		{
			...
			using (this.outputStream_ = new ZipOutputStream(outputStream))
			{
				...
				{
					FileSystemScanner expr_88 = fileSystemScanner;
					expr_88.ProcessDirectory = (ProcessDirectoryHandler)Delegate.Combine(expr_88.ProcessDirectory, new ProcessDirectoryHandler(this.ProcessDirectory));
				}
				if (this.events_ != null)
				{
					if (this.events_.FileFailure != null)
					{
						FileSystemScanner expr_C5 = fileSystemScanner;
						expr_C5.FileFailure = (FileFailureHandler)Delegate.Combine(expr_C5.FileFailure, this.events_.FileFailure);
					}
					if (this.events_.DirectoryFailure != null)
					{
						FileSystemScanner expr_F6 = fileSystemScanner;
						expr_F6.DirectoryFailure = (DirectoryFailureHandler)Delegate.Combine(expr_F6.DirectoryFailure, this.events_.DirectoryFailure);
					}
				}
		...
		public void ExtractZip(Stream inputStream, string targetDirectory, FastZip.Overwrite overwrite, FastZip.ConfirmOverwriteDelegate confirmDelegate, string fileFilter, string directoryFilter, bool restoreDateTime, bool isStreamOwner)
		{
			...
			using (this.zipFile_ = new ZipFile(inputStream))
			{
				...
				while (this.continueRunning_ && enumerator.MoveNext())
				{
					...
					{
						if (zipEntry.IsDirectory && this.directoryFilter_.IsMatch(zipEntry.Name) && this.CreateEmptyDirectories)
						{
							this.ExtractEntry(zipEntry);
						}
					}
